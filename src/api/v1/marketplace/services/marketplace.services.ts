...
export class MarketplaceServices {
    ...
    const result = await axios(config)
      .then(function (response) {
        ...
        const json = {
          ...
          contractAddress:
            data.collection.primary_asset_contracts[0]?.address?.toString(),
  ...
  static getNextCollectionNFTsFromSlug = async (
    slug: string,
    offset: number
  ) => {
  ...
  static getCollectionNFTsFromSlugService = async (
    slug: string,
    limit: number
  ) => {
    let corsURL = "https://cors.renderverse.workers.dev/?u=";
    let url = encodeURIComponent(
      "https://api.opensea.io/api/v1/assets?collection_slug=" +
        slug +
        "&limit=" +
        limit +
        "&order_direction=asc"
  ...
  static getCollectionNFTsFromSymbolService = async (
    symbol: string,
    limit: number
  ) => {
    let corsURL = "https://cors.renderverse.workers.dev/?u=";
  ...
  static getNFTListingsService = async (contract: string, token_id: string) => {
    ...
    const result = await axios(config)
      .then(async function (response) {
        ...
        const price = (
          (JSON.parse(JSON.stringify(response.data)).seaport_listings[0]
            ?.current_price ?? 0) /
          10 ** 18
        )?.toString();
        return price;
  ...
  static getNFTFromContractService = async (
    contract: string,
    token_id: string
  ) => {
    let corsURL = "https://cors.renderverse.workers.dev/?u=";
    ...
    const result = await axios(config)
      .then(async function (response) {
        const data = JSON.parse(JSON.stringify(response.data));
        const creator = await MarketplaceServices.convertJSONToStringValues(
          data.creator
        );
        const owner = await MarketplaceServices.convertJSONToStringValues(
          data.owner
        );
        const json = {
          ...
          totalSupply: data.collection.stats.total_supply?.toString(),
          traits: await MarketplaceServices.convertJSONToStringValues(
            data.traits
          ),
          creator: creator ?? {
            user: {
            ...
            address: "0x07ed64bbd9d6a881b951ce758ae867a0c112188e",
            config: "",
          },
          owner: owner ?? {
            user: {
  ...
  static getTrendingCollectionData = async (
    category: string,
    chain: string,
    count: number
  ) => {
    try {
      ...
      const collectionDataFilePath = path.join(
        LOCAL_DATA_FOLDER_PATH,
        collectionDataFileName
      );
  ...
  static getEthereumShowcaseNFTsService = async (limit: number) => {
    try {
      ...
      const showcaseSlugsFilePath = path.join(
        LOCAL_SLUGS_FOLDER_PATH,
        showcaseSlugsFileName
      );
      var fs = require("fs");
      var slugs = fs
        .readFileSync(showcaseSlugsFilePath)
        ?.toString()
        .split("\n");
      let i = 0;
      let j = 0;
      ...
      while (i < limit) {
        var slug = slugs[j];
        const nfts = (
          await this.getCollectionNFTsFromSlugService(slug.trim(), 10)
        ).slice(3);
        for (let nft of nfts) {
          results.push(nft);
          i += 1;
  ...
  static getSolanaShowcaseNFTsService = async (limit: number) => {
    try {
      ...
      const solanaSymbolsFilePath = path.join(
        LOCAL_SLUGS_FOLDER_PATH,
        solanaSymbolsFileName
      );
      var fs = require("fs");
      var symbols = fs
        .readFileSync(solanaSymbolsFilePath)
        ?.toString()
        .split("\n");
      let i = 0;
      let j = 0;
      ...
      while (i < limit) {
        ...
        j += 1;
        const nfts = await this.getCollectionNFTsFromSymbolService(
          symbol.trim(),
          2
        );
        for (let nft of nfts) {
  ...
  static getNotableCollectionService = async (limit: number) => {
    try {
      ...
      const notableSlugsFilePath = path.join(
        LOCAL_SLUGS_FOLDER_PATH,
        notableSlugsFileName
      );
      var fs = require("fs");
      var slugs = fs
        .readFileSync(notableSlugsFilePath)
        ?.toString()
        .split("\n")
      ...
      for (let i = 0; i < slugs.length; i++) {
        const info = await this.getCollectionInfoFromSlugService(
          slugs[i].trim()
        );
        const json = {
  ...
  static searchCollectionsService = async (
    searchstr: string,
    limit: number
  ) => {
    let url =
      "https://ubiquity.api.blockdaemon.com/v1/nft/ethereum/mainnet/collections/search?name=" +
      searchstr +
      "&verified=true";
  ...
  static searchNFTService = async (searchstr: string, limit: number) => {
    let url =
      "https://api.nftport.xyz/v0/search?text=" +
      searchstr +
      "&chain=ethereum&page_number=1&page_size=" +
      limit +
      "&order_by=mint_date&sort_order=desc";
    var config = {
      method: "get",
      url: url,
      headers: {
